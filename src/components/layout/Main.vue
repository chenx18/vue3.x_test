...
export default {
  ...
  setup(props, { emit }) {
    ...
    function handleLayoutData() {
      ...
      tabData.value.forEach((item) => {
        if (path === item.path) {
          ...
          newArr.push({ path: item.path, name: item.name, tagClass: "tag cursorP mr15" });
        }
      });
      !flag ? newArr.push({ path, name, tagClass: "tag cursorP mr15 tag-success" }) : null;
    ...
    function handleColseTag(i) {
      ...
      newTags.splice(i, 1);
      if (newTags.length === 0) {
        newTags = [{name: "首页",path: "/layout/home"}];
      }
      tabData.value = newTags;
      router.value.push(tabData.value[tabData.value.length - 1].path);
    }
